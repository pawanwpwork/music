function createCalendar(e, t, a) {
    function n() {
        ...
        for (var l = 0; l < s.length - 3; l++) {
            ...
            else if (l == s.length - 4) i.className += " cld-fwd cld-nav", i.addEventListener("click", function() {
                "function" == typeof e.Options.ModelChange ? e.Model = e.Options.ModelChange() : e.Model = e.Options.ModelChange, createCalendar(e, t, 1)
            ...
            else if (4 > l ? i.className += " cld-pre" : l > 4 ? i.className += " cld-post" : i.className += " cld-curr", function() {
                    var a = l - 4;
        ...
        if (a.className += "cld-datetime", e.Options.NavShow && !e.Options.NavVertical) {
            ...
            n.className += " cld-rwd cld-nav", n.addEventListener("click", function() {
                ...
                $('.cld-day').click(function(){
                    ...
                    var ajax_filter_url = $("input[name=ajax_filter_url]").val();
    ...
    function i() {
        ...
        for (var t = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"], a = 0; a < t.length; a++) {
    ...
    function d() {
        ...
        for (var n = 0; n < e.Selected.Days; n++) {
            var l = document.createElement("li");
            l.className += "cld-day currMonth "+e.Selected.Days;
            for (var i = (n + e.Selected.FirstDay) % 7, d = 0; d < e.Options.DisabledDays.length; d++) i == e.Options.DisabledDays[d] && (l.className += " disableDay");
            	var calendarDate = n + 1;
            	if(calendarDate <= 9){
                    if(parseInt(e.Selected.Month) < 9)
                    {
                        var calendarDateFormat = e.Selected.Year+'-0'+(e.Selected.Month + 1)+'-0'+(n + 1);    
                    }
                    else
                    {
                        var calendarDateFormat = e.Selected.Year+'-'+(e.Selected.Month + 1)+'-0'+(n + 1);    
                    }
                    ...
                    if(parseInt(e.Selected.Month) < 9)
                    {
                        var calendarDateFormat = e.Selected.Year+'-0'+(e.Selected.Month + 1)+'-'+(n + 1);
                    }
                    else
                    {
                        var calendarDateFormat = e.Selected.Year+'-'+(e.Selected.Month + 1)+'-'+(n + 1);  
                    }
            	}
            	l.setAttribute("id", calendarDateFormat);
